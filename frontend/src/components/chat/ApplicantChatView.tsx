...
import { InstitutionSelector } from '@/components/chat/InstitutionSelector';
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
export const ApplicantChatView = () => {
    ...
    const {
        ...
        queryFn: messageAPI.getAvailableOrganizations,
    });
    ...
    const createChatMutation = useMutation({
        mutationFn: (organizationId: string) => messageAPI.createChat(organizationId),
        onSuccess: (newChat) => {
    ...
    const filteredChats = chats.filter((chat: Chat) =>
        chat.organization_name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    const handleSelectInstitution = (organizationId: string) => {
        const existingChat = chats.find((chat: Chat) => chat.organization.id === organizationId);
        if (existingChat) {
    ...
    }, [chatsError, organizationsError, messagesError, toast]);
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center">
                                                    {chat.organization_name.charAt(0)}
                                                </div>
                                                <div className="flex-1">
                                                    <p className="font-medium">{chat.organization_name}</p>
                                                    <p className="text-sm text-gray-500">{chat.organization_name.slice(0, 3).toUpperCase()}</p>
                                                </div>
                                            </div>
                                            {unreadCount > 0 && (
                                ...
                                {isLoadingMessages ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center h-full py-12 text-center text-gray-500">
                                        <p className="text-lg font-medium mb-2">
                                            Начните общение с {selectedChat?.organization_name}
                                        </p>
                                        <p className="max-w-md">
                                            Задайте вопрос о поступлении, программах обучения или студенческой жизни
                                        </p>
                                    </div>
                                )}
                            </ScrollArea>
