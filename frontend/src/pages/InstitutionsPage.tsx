...
import { Building, Plus, Search, MapPin, Mail, Phone, Trash2 } from 'lucide-react';
...
// Схема валидации, синхронизированная с моделями
const organizationSchema = z.object({
    ...
    buildings: z
        .array(
            z.object({
                id: z.number().optional(), // ID для существующих корпусов при редактировании
                name: z.string().min(2, 'Название корпуса должно содержать минимум 2 символа').max(100, 'Название корпуса не должно превышать 100 символов'),
...
const InstitutionsPage = () => {
    ...
    const onSubmit = async (data: OrganizationForm) => {
        try {
            if (editingOrg) {
                console.log('Editing organization with ID:', editingOrg.id); // Добавлено для отладки
                ...
                for (const building of data.buildings) {
                    if (building.id) {
                        ...
                        // Проверяем, что editingOrg.id существует
                        if (!editingOrg.id) {
                            throw new Error('ID организации отсутствует');
                        }
                        // Создаём новый корпус
                ...
                // Добавляем новую организацию с корпусами
                setOrganizations((prev) => [
                    ...prev,
            ...
            console.error('Ошибка при создании/обновлении:', error); // Добавлено для отладки
            toast({
                title: 'Ошибка',
